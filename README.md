# grocery-cart-simulation

A simple command-line grocery store simulation written in Java. Users can browse products, add/remove items from a shopping cart, view cart details with tax calculation, and complete a checkout with a printed receipt.

Features

- Browse available grocery items with prices
- Add products to your shopping cart by number
- Remove items from the cart
- View cart with subtotal, 8% tax, and total
- Checkout and print receipt
- Input validation for all menu choices

Requirements

- Java 17 or higher
- Any OS
- IntelliJ IDEA or any Java IDE

How to Run the Program

IntelliJ IDEA

1. Open IntelliJ IDEA
2. Click File -> Open and select the folder containing the project
3. Wait for IntelliJ to index and load the files
4. In the Project view, navigate to Main.java
5. Right-click on Main.java 
6. Select Run
7. The program will start in the terminal window inside IntelliJ

Option 2: Run from the Command Line

On Windows

1. Open Command Prompt or PowerShell
2. Navigate to the folder where your .java files are located:
   ```cmd
   cd path\to\your\project\folder
